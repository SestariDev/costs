        ...
        <div key={sub.codigo} className='flex flex-col'>
          <div className='flex items-center justify-between'>
            <div className='flex items-center gap-3'>
              <span
                className={` ${sub.categoria !== 'A' ? 'font-bold' : ''}`}
              >
                {sub.categoria}
              </span>
              {sub.categoria !== 'A' ?
              <button 
                className={` rounded-md flex justify-center p-1 items-center text-cente ${!visibleSubcontas[sub.codigo] ? 'bg-green-400' : 'bg-red-500'} `}
              ...
              <span className='ml-[37px]'> {sub.codigo}</span>
            </div>
  ...
  useEffect(() => {
    setTimeout(() => {
      fetch('http://localhost:5000/contas', {
        ...
        .then((resp) => resp.json())
        .then((data) => {
          updateValues(data, editedValues);
          setData(data);
        })
